...
Route::middleware('cors')->group(function () {
    ...
    Route::group(['middleware' => 'auth'], function () {
        ...
        Route::get('/cart/error/{email}/{cost}', [CartController::class, 'error'])
            ->name('cart.error');
        Route::get('/cart/checkout/', [CartController::class, 'checkout'])
            ->name('cart.checkout');
        Route::post('/cart/finishPayment/', [CartController::class, 'finishPayment'])
            ->name('cart.finishPayment');
        Route::get('/cart/paymentError/', [CartController::class, 'paymentError'])
            ->name('cart.paymentError');
        Route::get('/cart/noCredit/', [CartController::class, 'noCredit'])
            ->name('cart.noCredit');
        Route::get('/cart/showPaymentError/', [CartController::class, 'showPaymentError'])
            ->name('cart.showPaymentError');
    });
